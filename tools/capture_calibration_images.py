...
# LICENSE-sboys3-camera-calibration). Changes: the camera is opened through
# gxr_common.open_camera so resolution/exposure/fourcc are the pinned values
# from gxr_camera.json - the calibration is only valid for the pixels the
# tools later see. Everything else is the original workflow: press c to
# capture, q to quit; the board (board.svg) is a 10x7 inner-corner chessboard.
import cv2
import os
import time
...
def capture_calibration_images():
    ...
    # Open camera with the pinned settings (gxr_camera.json)
